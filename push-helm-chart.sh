...
aws ecr create-repository --repository-name helm/${CHART_NAME} --region ${AWS_REGION} || true
...
# Push to ECR with helm prefix
helm push ${CHART_NAME}-${CHART_VERSION}.tgz oci://${ECR_REGISTRY}/helm

echo "Chart pushed to: oci://${ECR_REGISTRY}/helm/${CHART_NAME}:${CHART_VERSION}"
